...
def convert_dataset_to_examples(datasets, mode, history_turn=-1):
    ...
    for index in trange(len(datasets[mode])):
        ...
        if eval(data_future['id'].split('#')[-1]) > eval(num):
            future_label = True
        ...
        if future_label:
            future_list.append([data_future['orig_answer']['answer_start'], None, data_future['orig_answer']['text']])
...
if __name__ == '__main__':
    ...
    features, dataset = convert_examples_to_features(examples, tokenizer=tokenizer, max_seq_length=config['seq_length'], doc_stride=config['stride'],  is_training=config['is_training'])
