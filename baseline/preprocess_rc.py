...
def convert_examples_to_features(
    ...
    with Pool(threads, initializer=convert_example_to_features_init, initargs=(tokenizer,)) as p:
        ...
        features = list(
            tqdm(
                p.imap(annotate_, examples, chunksize=1),
                total=len(examples),
...
if __name__ == "__main__":
    ...
    max_query_length = 64
    is_training = False
    cached_features_file = "doqa_test_file_movie"
    mode = "test"

    tokenizer = BertTokenizer.from_pretrained("bert-base-uncased")
    doqa_dataset = load_dataset("doqa","movies",cache_dir="./doqa")

    model = BertModel.from_pretrained('bert-base-uncased',return_dict=True)

    examples = convert_dataset_to_examples(doqa_dataset, mode)
    features, dataset = convert_examples_to_features(examples, tokenizer=tokenizer, max_seq_length=max_length, doc_stride=stride, max_query_length=max_query_length, is_training=is_training)
    start = time.time()
    torch.save(
