...
def convert_example_to_features(example, tokenizer, padding_strategy, is_training, doc_stride=128):
    ...
    future_span_modify = []
    # handle history answer embedding
    for item in example.future_span_list:
        if item[0] == -1:
            continue
        future_start_position = orig_to_tok_index[item[0]]
        if item[1] < len(example.doc_tokens) - 1:
            future_end_position = orig_to_tok_index[item[1]+1] - 1
        else:
            future_end_position = len(all_doc_tokens) - 1
        
        (tok_future_start_position, tok_future_end_position) = _improve_answer_span(
            all_doc_tokens, future_start_position, future_end_position, tokenizer, item[2]
        )
        future_span_modify.append((tok_future_start_position, tok_future_end_position))     
    ...
    for span in spans:
        ...
        future_in_context =[]
        for index in range(len(future_span_modify)):
            doc_start = span["start"]
            doc_end = span["start"] + span["length"] - 1
            doc_offset = sequence_added_tokens
            if not future_span_modify[index][0] >= doc_start and future_span_modify[index][1] <= doc_end:
                continue
            else:
                start_future_position = future_span_modify[index][0] - doc_start + doc_offset
                end_future_position = future_span_modify[index][1] - doc_start + doc_offset
                future_in_context.append((start_future_position, end_future_position))
...
class RCExample:
    ...
    def __init__(
        ...
        # handle future span list
        for i in range(len(self.future_origin)):
            if self.future_origin[i][2] == "CANNOTANSWER":
                start_position_history = -1
                end_position_history = -1
            else:
                start_position_history = char_to_word_offset[self.future_origin[i][0]]
                end_position_history = char_to_word_offset[min(self.future_origin[i][0] + len(self.future_origin[i][2]) - 1, len(char_to_word_offset)-1)]
            self.future_span_list.append([start_position_history, end_position_history, self.future_origin[i][2]])        
...
def convert_dataset_to_examples(datasets, mode,future=False):
    ...
    for index in range(len(datasets[mode])):
        ...
        #assure future answer is existed!
        future_label = False
        if index < len(datasets[mode])-1:
            data_future = datasets[mode][index+1]
            is_impossible_future = True if data_future['orig_answer']['text'][0] =="CANNOTANSWER" else False
            answer_text_future = data_future['orig_answer']['text'][0] if not is_impossible_future else "CANNOTANSWER"
            start_position_character_future = data_future['orig_answer']['answer_start'][0]


        if eval(data_future['id'].split('#')[-1]) == eval(num)+1:
            future_label=True
        else:
            is_impossible_future =  True if data['orig_answer']['text'][0] =="CANNOTANSWER" else False
            answer_text_future =  "CANNOTANSWER"
            start_position_character_future = -1
        ...
        if future_label:
            future_list.append([data_future['orig_answer']['answer_start'][0], None, data_future['orig_answer']['text'][0]])
        else:
            future_list.append([-1, None, "CANNOTANSWER"])

        
        if index <len(datasets[mode])-1 or not future:
            example = RCExample(qas_id=data['id'],
                                question_text=question,
                                context_text=data["context"],
                                dialog_act=dialog_act,
                                answer_text=answer_text if not future else answer_text_future,
                                start_position_character=start_position_character if not future else start_position_character_future, 
                                title=data['title'],
                                is_impossible=is_impossible,
                                answers=answers,
                                history_span_list=history_span_list,
                                future_span_list=future_list)
...
class RCFeatures:
    ...
    def __init__(
        ...
        self.future_start_list = []
        self.future_end_list = []

        for item in future_span_list:
            self.future_start_list.append(item[0])
            self.future_end_list.append(item[1])            
...
def extract_and_save_feature(dataset, mode, tokenizer, is_training, name, ratio, is_dev, stride=128,future=False):
    
    examples = convert_dataset_to_examples(dataset,mode,future)
...
if __name__ == "__main__":
    ...
    extract_and_save_feature(dataset, config['mode'], tokenizer, config['is_training'], cached_features_file, ratio=config['ratio'], is_dev=config['is_dev'],future=args.future)
