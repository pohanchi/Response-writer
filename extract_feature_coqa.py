    ...
    for index in trange(len(datasets[mode])):
        ...
        if eval(num) != 0:
            ...
            for i in range(previous,1,1):
                ...
                if i !=0:
                    question = question + history['question'] + " " + history['answers'][0]['text'] + " " +"[SEP]" + " "
                else:
...
if __name__ == '__main__':
    ...
    stride = 128
    ratio = 1
    cached_features_file = "coqa/coqa_train_try_file_sep"
    ...
    for pa_pairs in tqdm(data):
        ...
        for idx in range(len(pa_pairs['questions'])):
            ...
            if pa_pairs['answers'][idx]['span_text'] != "unknown":
                ...
                data_pair['answers'] = [{'text':pa_pairs['answers'][idx]['span_text'], 'answer_start':pa_pairs['answers'][idx]['span_start']}]
            else:

                data_pair['dialog_act'] = 0
                data_pair['answers'] = [{'text':"unknown", 'answer_start': -1}]
                data_pair['is_impossible'] = True
            
            if mode != "train":
            
                for str_keys in list(pa_pairs['additional_answers'].keys()):

                    if pa_pairs['additional_answers'][str_keys][idx]['span_text'] != "unknown":

                        if pa_pairs['additional_answers'][str_keys][idx]['input_text'].lower() == "yes" or pa_pairs['additional_answers'][str_keys][idx]['input_text'].lower() == "no":
                            data_pair['dialog_act'] = 1 if  pa_pairs['additional_answers'][str_keys][idx]['input_text'].lower() == "yes" else 2
                            data_pair['is_impossible'] = True
                        else:
                            data_pair['dialog_act'] = 3
                            data_pair['answers'].append([{'text':pa_pairs['additional_answers'][str_keys][idx]['span_text'], 'answer_start':pa_pairs['additional_answers'][str_keys][idx]['span_start']}])
                    else:

                        data_pair['dialog_act'] = 0
                        data_pair['is_impossible'] = True
            
            data_pair['context'] = context
