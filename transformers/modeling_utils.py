...
class ModuleUtilsMixin:
    ...
    def get_extended_attention_mask_cross_attention(self, attention_mask: Tensor, input_shape: Tuple[int],q_seg: Tensor, device: device) -> Tensor:
        """
        Makes broadcastable attention and causal masks so that future and masked tokens are ignored.
...
def apply_chunking_to_forward_memory(
    ...
    Examples::
        ...
        # rename the usual forward() fn to forward_chunk()
        def forward_chunk(self, hidden_states, mem):
            hidden_states = self.decoder(hidden_states)
            return hidden_states

        # implement a chunked forward function
        def forward(self, hidden_states, mem):
            return apply_chunking_to_forward_memory(self.forward_chunk, self.chunk_size_lm_head, self.seq_len_dim, hidden_states)
    """
