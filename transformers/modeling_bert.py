...
class BertEmbeddingsMemory3(nn.Module):
    """Construct the embeddings from word, position and token_type embeddings."""
    ...
    def forward(self, input_ids=None, token_type_ids=None, position_ids=None, inputs_embeds=None, memory_ids=None, memory_embed=None, memory=False):
        ...
        if memory == True:
            ...
            embeddings = memory_embed
            embeddings = self.LayerNorm(embeddings + inputs_embeds + position_embeddings + memory_embed)
            embeddings = self.dropout(embeddings)
            return embeddings
...
class BertEmbeddings_initial_from_pretrained_space(nn.Module):
    ...
    def _resize_position_embeddings(self, old_embeddings: torch.nn.Embedding, new_num_tokens,device):
        old_num_tokens, old_embedding_dim = old_embeddings.weight.size()
        new_embeddings = nn.Embedding(new_num_tokens, old_embedding_dim).to(device)
        mean = old_embeddings.weight[:self.max_position_embeddings].mean(0)
        variance = old_embeddings.weight[:self.max_position_embeddings].var(0)
        new_embeddings_init = torch.normal(mean,variance)
        new_embeddings.weight.data[:] = new_embeddings_init
        new_embeddings.weight.data[:old_embeddings.weight.shape[0],:] = old_embeddings.weight.data[:old_embeddings.weight.shape[0],:] 
    ...
    def forward(self, input_ids=None, token_type_ids=None, position_ids=None, inputs_embeds=None):
        ...
        if position_ids is None:
            if seq_length < self.position_embeddings.weight.shape[0]:
                position_ids_tmp=torch.arange(seq_length).expand((1, -1)).to(input_ids.device)
                position_ids = position_ids_tmp[:, :seq_length]

            else:
                self.position_embeddings = self._resize_position_embeddings(self.position_embeddings,seq_length,self.position_ids.device)
                position_ids_tmp=torch.arange(seq_length).expand((1, -1)).to(input_ids.device)
                position_ids = position_ids_tmp[:, :seq_length]
...
class BertSelfAttentionMemory(nn.Module):
    ...
    def forward(
        ...
        if encoder_hidden_states is not None:
            ...
            # mixed_value_layer = torch.bmm(mixed_value_layer * self.m(mixed_value_layer), mem)

            # mixed_value_layer = mixed_value_layer.reshape(mixed_value_layer.shape[0]*mixed_value_layer.shape[1], mem.shape[2], int(mixed_value_layer.shape[2]/mem.shape[2]))
            # mixed_value_layer = torch.bmm(mem, mixed_value_layer)
            # mixed_value_layer = mixed_value_layer.reshape(hidden_states.shape[0],hidden_states.shape[1],hidden_states.shape[2])
...
class BertSelfOutputMemoryWHistory(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.dense = nn.Linear(config.hidden_size, config.hidden_size)
        self.LayerNorm = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.memory_module = BertHistoryGenerator8(config)


    def forward(self, hidden_states, input_tensor, memory, input_len, memory_len):
        hidden_states = self.dense(hidden_states)
        hidden_states, _ = self.memory_module(hidden_states, memory,memory_len, input_len)
        hidden_states = self.dropout(hidden_states)
        hidden_states = self.LayerNorm(hidden_states + input_tensor)
        return hidden_states
...
class BertAttentionMemory4(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.self = BertSelfAttentionMemory(config)
        self.output = BertSelfOutput(config)
    ...
    def forward(
        ...
        if mem is not None:
            attention_output = self.output(self_outputs[0], hidden_states + mem)
        else:
            attention_output = self.output(self_outputs[0], hidden_states)
...
class BertAttentionMemory2(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.self = BertSelfAttentionMemory(config)
        self.output = BertSelfOutputMemoryWHistory(config)
        self.pruned_heads = set()
        self.m = nn.Softmax(dim=1)
        self.m_ = nn.Softmax(dim=0)
    ...
    def forward(
        ...
        attention_output = self.output(self_outputs[0], hidden_states, mem, input_len, memory_len)
        outputs = (attention_output, ) + self_outputs[2:]  # add attentions if we output them
        return outputs
...
class BertAttentionMemoryReuse(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.self = BertSelfAttentionReuse(config)
        self.output = BertSelfOutput(config)
        self.pruned_heads = set()
        self.m = nn.Softmax(dim=1)
        self.m_ = nn.Softmax(dim=0)
...
class BertOutputMemory2(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.dense = nn.Linear(config.intermediate_size, config.hidden_size)
        self.LayerNorm = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.memory_module = BertHistoryGenerator8(config)
    def forward(self, hidden_states, input_tensor, mem, input_len, memory_len):
        hidden_states = self.dense(hidden_states)
        hidden_staets, _ = self.memory_module(hidden_states, mem, memory_len, input_len)
        hidden_states = self.dropout(hidden_states)
        hidden_states = self.LayerNorm((hidden_states + input_tensor))
...
class BertLayerMemory4(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.chunk_size_feed_forward = config.chunk_size_feed_forward
        self.seq_len_dim = 1
        self.attention = BertAttentionMemory4(config)
        self.is_decoder = config.is_decoder
        self.add_cross_attention = config.add_cross_attention
        if self.add_cross_attention:
            # assert self.is_decoder, f"{self} should be used as a decoder model if cross attention is added"
            self.crossattention = BertAttentionMemory4(config)
        self.intermediate = BertIntermediate(config)
        self.output = BertOutputMemory(config)
...
class BertLayerMemory2(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.chunk_size_feed_forward = config.chunk_size_feed_forward
        self.seq_len_dim = 1
        self.attention = BertAttentionMemory2(config)
        self.is_decoder = config.is_decoder
        self.add_cross_attention = config.add_cross_attention
        if self.add_cross_attention:
            # assert self.is_decoder, f"{self} should be used as a decoder model if cross attention is added"
            self.crossattention = BertAttentionMemory2(config)
        self.intermediate = BertIntermediate(config)
        self.output = BertOutputMemory2(config)
    ...
    def forward(
        ...
        intermediate_output = apply_chunking_to_forward_memory(
            self.feed_forward_chunk, self.chunk_size_feed_forward, self.seq_len_dim, attention_output)

        layer_output = self.output(intermediate_output, attention_output, mem, input_len, memory_len)
...
class BertLayerMemoryReuse(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.chunk_size_feed_forward = config.chunk_size_feed_forward
        self.seq_len_dim = 1
        self.attention = BertAttentionMemoryReuse(config)
        self.is_decoder = config.is_decoder
        self.add_cross_attention = config.add_cross_attention
        if self.add_cross_attention:
            # assert self.is_decoder, f"{self} should be used as a decoder model if cross attention is added"
        ...
        self.intermediate = BertIntermediate(config)
        self.output = BertOutputMemory(config)
        self.memory_module = BertHistoryGeneratorreuse(config, config.hidden_size)
    ...
    def forward(
        ...
        **kwargs,
        ...
        self_mem_outputs = self.attention(
            mem,
            hidden_states,
            relative_embeddings,
            memory_att_mask,
        ...
        self_mem_output = self_mem_outputs[0]
        ...
        attention_output, _ =self.memory_module(attention_output, self_mem_output, memory_len, 0)
        ...
        layer_mem_output = apply_chunking_to_forward_memory(
            self.feed_forward_chunk, self.chunk_size_feed_forward, self.seq_len_dim, self_mem_output)

        outputs = (layer_output, layer_mem_output) + outputs
        return outputs
...
class BertEncoderMemory4(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([BertLayerMemory4(config) for _ in range(config.num_hidden_layers)])
...
class BertEncoderMemory2(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([BertLayerMemory2(config) for _ in range(config.num_hidden_layers)])
...
class BertEncoderMemoryreuse(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([BertLayerMemoryReuse(config) for _ in range(config.num_hidden_layers)])
...
class BertHistoryGenerator24(nn.Module):
    def __init__(self, config, value_size):
        super().__init__()
        self.config = config
        self.memory_query = nn.ModuleList([nn.Linear(config.hidden_size, config.hidden_size) for _ in range(1)])
    ...
    def memory_operator(self, hidden_states, memory, history_len, input_len, index):
        ...
        seq_len = hidden_states.shape[1] 
        hidden_size  = hidden_states.shape[2]
        output1 = self.memory_query[index](hidden_states * global_memory_vertical_matrix1) #+ global_memory_lambda_matrix[:,:,0][:,None]
        hid = self.activation(output1)
        output2 = self.memory_query_vertical[index](hid * global_memory_vertical_matrix2) #+ global_memory_lambda_matrix[:,:,1][:,None]

        output = output2
        


        return output, global_memory_vertical_matrix
...
class BertHistoryGeneratorreuse(nn.Module):
    def __init__(self, config, value_size):
        super().__init__()
        self.config = config
        self.memory_value = nn.Linear(config.hidden_size, config.hidden_size)
        self.memory_key = nn.Linear(config.hidden_size, 1)
        self.m_ = nn.Softmax(dim=0)
        self.m2 = nn.Softmax(dim=1)
    
    def forward(self,hidden_states, memory, memory_len, index, last=False):

        hidden_states_tmp1, lambda_matrix = self.memory_operator(hidden_states, memory, memory_len, index)
        hidden_states_tmp2 = hidden_states_tmp1 + hidden_states

        return hidden_states_tmp2, lambda_matrix


    def memory_operator(self, hidden_states, memory, history_len, index):
...
class BertModelMemory6(BertPreTrainedModel):
    """
    ...
    def __init__(self, config):
        ...
        self.encoder = BertEncoderMemory2(config)
        self.encoder_generator = BertEncoderMemory(config)
        self.pooler = BertPooler(config)
        self.memory_module = BertHistoryGenerator8(config)
        self.init_weights()
    ...
    def forward(
        ...
        **kwargs,
        ...
        memory_embed_tmp = self.embeddings_mem(input_ids=memory, token_type_ids=memory_segment, memory_ids=memory_input_ids1,memory=False)
        memory_embed_query = self.embeddings_mem(input_ids=memory_query, token_type_ids=memory_segment_query, memory_ids=None,memory=False)
        memory_embed, _ = self.memory_module(memory_embed_tmp, memory_embed_query, memory_len_query, memory_len)
        embedding_output, _ = self.memory_module(embedding_output_tmp, memory_embed_tmp, memory_len, input_len)
...
class BertModelMemory7(BertPreTrainedModel):
    """
    ...
    def __init__(self, config):
        ...
        self.encoder_generator = BertEncoderMemory(config)
        self.pooler = BertPooler(config)
        self.memory_module = BertHistoryGenerator9(config)
        self.init_weights()
...
class BertModelreusemodule(BertPreTrainedModel):
    """
    ...
    def __init__(self, config):
        ...
        self.embeddings = BertEmbeddingsMemory2(config)
        self.encoder = BertEncoderMemoryreuse(config)
        self.pooler = BertPooler(config)
...
class BertModelinitial_from_mu_sigma(BertPreTrainedModel):
    """
    ...
    def __init__(self, config):
        ...
        self.embeddings = BertEmbeddings_initial_from_pretrained_space(config)
        self.encoder = BertEncoderRelative(config)
        self.pooler = BertPooler(config)
