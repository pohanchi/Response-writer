...
class BertInjectMemory(nn.Module):
    def __init__(self, config):
        ...
        self.align_query = nn.Linear(config.hidden_size, 12)
        self.align_key = nn.Linear(config.hidden_size,12)
        self.relative_embeddings = nn.Embedding(21, 768)

        self.lambda_convolution = nn.Conv1d(1, config.hidden_size, (1,21), bias=False,padding=(0,10))

        self.lambda_convolution.weight.data = self.relative_embeddings.weight.transpose(0,1).unsqueeze(1).unsqueeze(1).data
    ...
    def memory_operator(self, hidden_states, memory, history_len, input_len, query=None, key=None, value=None):
        ...
        global_lambda_matrix=torch.stack(memory_matrix).unsqueeze(1)

        new_feature=torch.bmm(self.m2(self.align_key(memory)), self.m2(self.align_query(hidden_states)).transpose(1,2))
        sec_hidden_states = torch.bmm(value(memory).transpose(1,2), new_feature)

        sec_hidden_states = sec_hidden_states.unsqueeze(1)

        local_lambda_function = self.lambda_convolution(sec_hidden_states)
        local_lambda_function = local_lambda_function.transpose(1,3)

        all_lambda_function = local_lambda_function + global_lambda_matrix
        ...
        if query is not None:
            ...
            # IPython.embed()
            # pdb.set_trace()
            all_lambda_function = all_lambda_function.reshape(hidden_states.shape[0]*hidden_states.shape[1], hidden_size, hidden_size)
            hidden_states = hidden_states.reshape(hidden_states.shape[0]*hidden_states.shape[1],hidden_states.shape[2]).unsqueeze(-1)
            # IPython.embed()
            # pdb.set_trace()
            output = torch.bmm(all_lambda_function, hidden_states).squeeze(-1)
...
class BertModelMemory(BertPreTrainedModel):
    """
