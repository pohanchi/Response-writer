...
class RobertaEmbeddingsRelative(nn.Module):
    """
    Same as BertEmbeddings with a tiny tweak for positional embeddings indexing.
    """
...
class RobertaEmbeddingsMemory(nn.Module):
    """
    Same as BertEmbeddings with a tiny tweak for positional embeddings indexing.
    """
    ...
    def forward(self, input_ids=None, token_type_ids=None, position_ids=None, inputs_embeds=None, memory_ids=None, memory=False):
        ...
        embeddings = inputs_embeds + position_embeddings + token_type_embeddings #+ memory_embeddings
        embeddings = self.LayerNorm(embeddings)
        embeddings = self.dropout(embeddings)
        return embeddings
    def create_position_ids_from_inputs_embeds(self, inputs_embeds):
...
class RobertaSelfAttentionRelative(nn.Module):
    def __init__(self, config):
        super().__init__()
        if config.hidden_size % config.num_attention_heads != 0 and not hasattr(config, "embedding_size"):
            raise ValueError(
    ...
    def forward(
        ...
        if self.relative_attention:
            ...
            rel_embedding = rel_embeddings(torch.arange(2,self.max_relative_positions*2+2).cuda())
    ...
    def disentangled_att_bias(self, query_layer, key_layer, relative_pos, rel_embeddings, scale_factor):
        ...
        if 'p2c' in self.pos_att_type or 'p2p' in self.pos_att_type:
            ...
            if query_layer.size(-2) != key_layer.size(-2):
                ...
                r_pos = relative_pos
            p2c_pos = torch.clamp(-r_pos + att_span, 0, 2*att_span-1)
            if query_layer.size(-2) != key_layer.size(-2):
...
class RobertaSelfAttentionMemory(nn.Module):
    def __init__(self, config):
        super().__init__()
        if config.hidden_size % config.num_attention_heads != 0 and not hasattr(config, "embedding_size"):
            raise ValueError(
...
class RobertaMemoryOutput(nn.Module):
    def __init__(self, config):
        super().__init__()
    ...
    def forward(self, hidden_states, input_tensor):
        # hidden_states1 = self.dense(hidden_states)
        # hidden_states2 = self.dropout(hidden_states)
        # hidden_states = self.InstanceNorm((hidden_states2 + input_tensor).transpose(1,2)).transpose(1,2)
...
class RobertaAttentionRelative(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.self = RobertaSelfAttentionRelative(config)
...
class RobertaAttentionMemory(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.self = RobertaSelfAttentionMemory(config)
        self.output = RobertaSelfOutput(config)
        self.pruned_heads = set()
...
class RobertaLayerRelative(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.chunk_size_feed_forward = config.chunk_size_feed_forward
        self.seq_len_dim = 1
        self.attention = RobertaAttentionRelative(config)
        self.is_decoder = config.is_decoder
        self.add_cross_attention = config.add_cross_attention
        if self.add_cross_attention:
            assert self.is_decoder, f"{self} should be used as a decoder model if cross attention is added"
            self.crossattention = RobertaAttentionRelative(config)
...
class RobertaLayerMemory(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.chunk_size_feed_forward = config.chunk_size_feed_forward
        self.seq_len_dim = 1
        self.attention = RobertaAttentionMemory(config)
        self.is_decoder = config.is_decoder
        self.add_cross_attention = config.add_cross_attention
        if self.add_cross_attention:
            assert self.is_decoder, f"{self} should be used as a decoder model if cross attention is added"
            self.crossattention = RobertaAttentionMemory(config)
        self.intermediate = RobertaIntermediate(config)
        self.output = RobertaOutput(config)
...
class RobertaEncoderRelative(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([RobertaLayerRelative(config) for _ in range(config.num_hidden_layers)])
...
class RobertaEncoderMemory(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([RobertaLayerMemory(config) for _ in range(config.num_hidden_layers)])
...
class RobertaModelRelative(RobertaPreTrainedModel):
    """
    ...
    def __init__(self, config):
        ...
        self.embeddings = RobertaEmbeddingsRelative(config)
        self.encoder = RobertaEncoderRelative(config)
        self.pooler = RobertaPooler(config)
...
class RobertaInjectMemory(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
...
class RobertaModelMemory(RobertaPreTrainedModel):
    """
...
class RobertaModelMemory2(RobertaPreTrainedModel):
    """
    ...
    def forward(
        ...
        embedding_output_tmp = self.embeddings(
            input_ids=input_ids, position_ids=position_ids, token_type_ids=token_type_ids, inputs_embeds=inputs_embeds
        ,memory_ids=None,memory=False)

        memory_embed = self.embeddings(input_ids=memory, token_type_ids=memory_segment, memory_ids=None,memory=False)
...
def create_position_ids_from_input_ids(input_ids, padding_idx):
    ...
    incremental_indices = torch.cumsum(mask, dim=1).type_as(mask) * mask -1 
    return incremental_indices.long() % 512 +2
