...
class RobertaInjectMemory(nn.Module):
    def __init__(self, config):
        ...
        self.memory_output = RobertaMemoryOutput(config)
        self.m_ = nn.Softmax(dim=0)
        self.m2 = nn.Softmax(dim=1)

    
    def forward(self,hidden_states, memory, memory_len):
        hidden_states_tmp1 = self.memory_operator(hidden_states, memory, memory_len, key=self.memory_key, query=self.memory_query, value=self.memory_value)
...
class RobertaModelMemory(RobertaPreTrainedModel):
    ...
    def forward(
        ...
        memory = self.embeddings(memory, token_type_ids=memory_segment,memory_ids=memory_input_ids1)
        embedding_output = self.memory_module(embedding_output_tmp, memory, memory_len)
        encoder_outputs = self.encoder(
            embedding_output,
            self.embeddings.position_embeddings,
            attention_mask=extended_attention_mask,
