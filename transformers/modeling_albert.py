...
class AlbertEmbeddingsRelative(nn.Module):
    """Construct the embeddings from word, position and token_type embeddings."""
...
class AlbertAttentionRelative(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class AlbertAttentionMemory(nn.Module):
    def __init__(self, config):
        super().__init__()
        if config.hidden_size % config.num_attention_heads != 0 and not hasattr(config, "embedding_size"):
            raise ValueError(
...
class AlbertLayerRelative(nn.Module):
    def __init__(self, config):
        super().__init__()
        ...
        self.seq_len_dim = 1
        self.full_layer_layer_norm = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        self.attention = AlbertAttentionRelative(config)
        self.ffn = nn.Linear(config.hidden_size, config.intermediate_size)
        self.ffn_output = nn.Linear(config.intermediate_size, config.hidden_size)
        self.activation = ACT2FN[config.hidden_act]
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
...
class AlbertLayerMemory(nn.Module):
    def __init__(self, config):
        super().__init__()
        ...
        self.seq_len_dim = 1
        self.full_layer_layer_norm = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        self.attention = AlbertAttentionMemory(config)
        self.ffn = nn.Linear(config.hidden_size, config.intermediate_size)
        self.ffn_output = nn.Linear(config.intermediate_size, config.hidden_size)
        self.activation = ACT2FN[config.hidden_act]
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
    ...
    def forward(
        self, hidden_states, relative_embeddings, memory,attention_mask=None, map_in_fn=None, head_mask=None, output_attentions=False, output_hidden_states=False
    ):
        attention_output = self.attention(hidden_states, relative_embeddings,memory, attention_mask, map_in_fn, head_mask, output_attentions)
...
class AlbertLayerGroupRelative(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class AlbertLayerGroupMemory(nn.Module):
    def __init__(self, config):
        super().__init__()

        self.albert_layers = nn.ModuleList([AlbertLayerMemory(config) for _ in range(config.inner_group_num)])
    ...
    def forward(
        ...
        for layer_index, albert_layer in enumerate(self.albert_layers):
            layer_output = albert_layer(hidden_states, relative_embeddings, memory,attention_mask, map_in_fn, head_mask[layer_index], output_attentions)
            hidden_states = layer_output[0]
...
class AlbertTransformerRelative(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class AlbertTransformerMemory(nn.Module):
    def __init__(self, config):
        ...
        self.config = config
        self.embedding_hidden_mapping_in = nn.Linear(config.embedding_size, config.hidden_size)
        self.memory_module = AlbertHistoryGenerator(config)
        self.albert_layer_groups = nn.ModuleList([AlbertLayerGroupMemory(config) for _ in range(config.num_hidden_groups)])
    ...
    def forward(
        ...
        hidden_states_tmp = self.embedding_hidden_mapping_in(hidden_states)
        memory = self.embedding_hidden_mapping_in(memory)
        memory_query = self.embedding_hidden_mapping_in(memory_query)

        memory_embedding_output, memory_lambda = self.memory_module(memory, memory_query, memory_query_len, memory_len,0)
        # memory_embedding_output = memory
        # hidden_states = hidden_states_tmp
        hidden_states, memory_lambda = self.memory_module(hidden_states_tmp, memory_embedding_output, memory_len, input_len,0)
        ...
        for i in range(self.config.num_hidden_layers):
            ...
            layer_group_output_mem = self.albert_layer_groups[group_idx](
                memory_embedding_output,
                relative_embeddings,
                hidden_states,
            ...
            hidden_states = layer_group_output[0]
            memory_embedding_output = layer_group_output_mem[0]
        ...
        hidden_states, _ =self.memory_module(hidden_states, memory_embedding_output, memory_len, input_len, 0)
...
class AlbertModelRelative(AlbertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.config = config
        self.embeddings = AlbertEmbeddingsRelative(config)
        self.encoder = AlbertTransformerRelative(config)
...
class AlbertHistoryGenerator(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.memory_query= nn.ModuleList([nn.Linear(config.hidden_size, config.hidden_size) for _ in range(1)])
...
class AlbertModelMemory(AlbertPreTrainedModel):
    ...
    def forward(
        ...
        memory_embed = self.embeddings(input_ids=memory,token_type_ids=memory_segment\
        ...
        encoder_outputs = self.encoder(
            ...
            memory_embed,
            memory_embed_query,
            memory_query_len=memory_query_len,
