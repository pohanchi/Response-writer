...
class BERTQA_memory_HAE_question(nn.Module):
    def __init__(self, config):
        super(BERTQA_memory_HAE_question, self).__init__()
        #BERT layer, n_dim, nhead
        config = Namespace(**config)
    ...
    def forward(self,q_ids, q_start, q_len, c_ids, c_att_masks, c_len, dialog_act,start_positions, end_positions, history_starts, history_ends,future_q, future_att, **kwargs):
...
class BERTQA_memoryHistory_question(nn.Module):
    def __init__(self, config):
        super(BERTQA_memoryHistory_question, self).__init__()
        ...
        self.pretrained_decoder_config.num_hidden_layers = 3
        self.pretrained_decoder = BertLMHeadModel.from_pretrained("bert-base-uncased", config=self.pretrained_decoder_config)
        self.pretrained_decoder.bert.embeddings = self.pretrained_model.embeddings

    def forward(self,q_ids, q_start, q_len, c_ids, c_att_masks, c_len, dialog_act,start_positions, end_positions,future_q,future_att, **kwargs):
...
class BERTQA_memory23_switch(nn.Module):
    def __init__(self, config):
        super(BERTQA_memory23_switch, self).__init__()
        # interact module over and over again
        #BERT layer, n_dim, nhead
        config = Namespace(**config)
        ...
        if "pretrained_name" not in list(vars(config).keys()):
            ...
            self.model_config.update({"bottleneck_size": config.bottleneck_size, "add_cross_attention":config.add_cross_attention})
            self.pretrained_model = BertModelMemory23.from_pretrained("bert-base-uncased", config=self.model_config)
            self.pretrained_model.encoder_mem = self.pretrained_model.encoder
            self.pretrained_model.embeddings_mem = self.pretrained_model.embeddings_mem
    ...
    def forward(self,q_ids, q_start, q_len, c_ids, c_att_masks, c_len, dialog_act,start_positions, end_positions, history_starts, history_ends,**kwargs):
        ...
        for i in range(len(q_ids)):
            ...
            memory = q_ids[i][q_start[i]:q_len[i]]
            #memory = q_ids[i][:q_len[i]]
            if q_start[i] == 1:
                sep = torch.LongTensor([102]).to(q_seg.device)
                memory = torch.cat((memory,sep),dim=0)
...
class BERTQA_question(nn.Module):
    def __init__(self, config):
        super(BERTQA_question, self).__init__()
    ...
    def forward(self,q_ids, q_start, q_len, c_ids, c_att_masks, c_len, dialog_act,start_positions, end_positions, future_starts, future_ends, future_q, future_att, **kwargs):
        ...
        if start_positions is not None and end_positions is not None:
            ...
            input_args2 = {
                "input_ids":future_q,
                "attention_mask":future_att,
                "encoder_hidden_states":last_reps[0],
                "encoder_attention_mask":reason_mask,
                "labels":future_q,
                "return_dict":False,
                }
                
            output = self.pretrained_decoder(**input_args2)
            lm_loss = output[0]
